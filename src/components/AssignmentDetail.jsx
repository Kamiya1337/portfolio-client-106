...
export default function AssignmentDetail({ project, onBack, previewData, onPreview, onClosePreview }) {
  return (
    <div className="min-h-screen px-5 py-10 sm:px-8 xl:px-12 xl:py-16">
      ...
      <article className="glass-panel mx-auto mt-8 max-w-[1220px] overflow-hidden rounded-[2rem]">
        <header className="grid gap-8 border-b border-border-light p-7 lg:grid-cols-12 lg:p-12">
          ...
          <div className="lg:col-span-8"><h1 className="font-display text-[clamp(2.5rem,5vw,5rem)] font-normal leading-[0.98] tracking-[-0.02em]">{project.title}</h1><p className="mt-6 max-w-3xl text-base leading-8 text-muted-dark">{project.shortDesc}</p></div>
        </header>
              ...
              <div className="flex items-center gap-3">
                {(!project.report || project.report === 'Sẽ cập nhật sau') ? (
                  ...
                  <a 
                    ...
                    className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-primary px-5 py-2.5 text-[10px] font-bold uppercase tracking-[0.1em] text-white transition duration-200 hover:bg-tertiary"
                  >
