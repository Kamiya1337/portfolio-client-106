  ...
  return (
    <div>
      ...
      <section className="px-5 py-20 sm:px-8 xl:px-12">
        <div className="glass-panel mx-auto grid max-w-[1220px] gap-10 rounded-[2rem] p-8 lg:grid-cols-2 lg:items-center lg:p-10"><div><p className="editorial-label">Academic Standards</p><h2 className="mt-4 font-display text-4xl font-normal tracking-[-0.02em]">Evidence before decoration.</h2><p className="mt-6 max-w-xl text-base leading-8 text-muted-dark">Mỗi hồ sơ ưu tiên tính chính xác của nội dung, khả năng truy xuất tài nguyên và sự minh bạch trong quá trình thực hiện.</p></div><div className="rounded-[1.5rem] bg-white/60 p-10 shadow-glass"><p className="editorial-label">Archive Scope</p><p className="mt-5 font-display text-7xl font-normal">{String(projects.length).padStart(2, '0')}</p><p className="mt-2 text-sm text-muted-dark">documented assignments</p></div></div>
      </section>
    </div>
  );
}
